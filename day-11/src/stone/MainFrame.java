...
public class MainFrame extends JFrame {
    ...
    public MainFrame() {
        ...
        System.out.println("历史最小步数：" + minCount + "步");//打印日志
    ...
    private void initImage() {
        ...
        this.add(countTxt);
//每次启动都会初始化界面，这里添加一个标签，展示历史最小步数，注意不是第一次玩游戏才展示
        if (minCount != 0) {
    ...
    private void writeFileScore(int count) {
        try(
                FileWriter fw = new FileWriter("stone/src/score.txt");
                BufferedWriter bw = new BufferedWriter(fw);
                ) {
            // 把当前步数写入到文件中去
            bw.write(count + "");//转成字符串写进去
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
    ...
    private int readFileScore() {
        try(
                FileReader fr = new FileReader("stone/src/score.txt");
                BufferedReader br = new BufferedReader(fr);
                ) {
            // 读取文件中的最小步数
            String line = br.readLine(); // "23" "0"读取的是字符串
            return Integer.valueOf(line);// 把字符串转换成数字。
        } catch (Exception e) {
            e.printStackTrace();
            return 0;
        }
